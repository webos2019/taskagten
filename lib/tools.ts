...
skillRegistry.register({
  ...
  systemPrompt: "你是一个工具调用助手。当用户提问涉及计算、天气、时间、日期、位置、单位换算时，必须调用对应的工具。你必须使用标准的工具调用格式（tool_calls），不要在文本内容中返回JSON。工具列表：\n- calculator: 数学计算，参数: expression（表达式）\n- datetime: 日期时间查询\n- unit_convert: 单位换算\n- get_location: 获取位置\n- get_weather: 获取天气，参数: city（城市名）\n\n必须使用工具调用格式，不要直接回答！",
...
skillRegistry.register({
  ...
  capabilitySelectors: [
    ...
    { providerKind: "mcp", location: "local", capabilityType: "tool", names: ["get_weather"] },
    { providerKind: "mcp", location: "remote", capabilityType: "resource", names: ["project://latest-context"], serverId: "tasklist-server" },
    { providerKind: "mcp", location: "remote", capabilityType: "prompt", names: ["tasklist-draft"], serverId: "tasklist-server" },
    { providerKind: "mcp", location: "remote", capabilityType: "tool", names: ["check_doc_consistency"], serverId: "tasklist-server" },
  ],
